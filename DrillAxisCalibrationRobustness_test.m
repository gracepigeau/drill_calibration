%Drill Axis Calibration Robustness Test
%CISC 330 - Computer Integrated Surgery 
%Assignment 3
%Grace Pigeau 10187678
%
%Purpose:   Test the robustness of the drill axis calibration function

function DrillAxisCalibrationRobustness_test(truth)

%initialize a matrix to hold the Emax values and the error they create
...
for ix = 0:0.2:4
    %generate 300 points with a range of 360 degrees
    trial = DrillAxisSimulator(100, 360);
    ...
    differences(2,diffCount) = DistBtwnPoints(Vm,truth);
    
    %record the first Emax where the difference between 
    %the calculated tip and the ground truth
    if (DistBtwnPoints(Vm,truth) > 0.1 && foundCutOff == 0)
        Emax = ix
        foundCutOff = 1;
    end
